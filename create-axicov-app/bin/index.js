...
`);
  ...
  if (!projectName) {
    const res = await import_prompts.default({
      type: "text",
      name: "projectName",
  ...
  const { runtime } = await import_prompts.default({
    type: "select",
    name: "runtime",
    message: "Runtime:",
    choices: [
  ...
  const { walletAddress } = await import_prompts.default({
    type: "text",
    name: "walletAddress",
...
`);
  const templateDir = join(__dirname2, "..", "template");
  ...
  if (runtime === "node") {
    dockerfile = dockerfile.replace("FROM oven/bun:1", "FROM node:22-alpine").replace("bun install", "npm install").replace('CMD ["bun", "run", "src/index.ts"]', 'CMD ["npx", "tsx", "src/index.ts"]');
  }
